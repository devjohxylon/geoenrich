...
def test_enrich_ip(monkeypatch):
    ...
    monkeypatch.setattr(
        "geoenrich.requests.get",
        lambda url, timeout=5: DummyResponse({
...
def test_enrich_coords(monkeypatch):
    ...
    monkeypatch.setattr(
        "geoenrich.requests.get",
        lambda url, params=None, timeout=5: DummyResponse(dummy_payload)
    )
    data = enrich_coords(40.7128, -74.0060)
