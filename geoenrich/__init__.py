...
def get_session(timeout: int, retries: int) -> requests.Session:
    ...
    def timed_request(method: str, url: str, **kwargs: Any) -> requests.Response:
        """Wrap the session request to include a timeout by default."""
        return original_request(method, url, timeout=timeout, **kwargs)

    session.request = timed_request  # type: ignore[method-assign, assignment]
    return session
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--ip-col', default='ip', help='Column name for IP addresses'
    )
    parser.add_argument(
